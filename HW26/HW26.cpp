...
// Функція для сумування елементів у векторі в однопоточному режимі
...
// Функція для сумування елементів у векторі в багатопоточному режимі (з діленням на підчастини)
...
int main() {
    // Генеруємо вектор даних різного розміру для порівняння
    std::vector<int> data1(1000000, 1); // Розмір вектора: 1 мільйон
    std::vector<int> data2(10000000, 1); // Розмір вектора: 10 мільйонів
    std::vector<int> data3(100000000, 1); // Розмір вектора: 100 мільйонів

    // Вимірюємо час виконання сумування в однопоточному режимі
    ...
    // Вимірюємо час виконання сумування в багатопоточному режимі (2 потоки)
    ...
    // Вимірюємо час виконання сумування в багатопоточному режимі (std::thread::hardware_concurrency() потоків)
    ...
    // Виводимо результати у табличку
    std::cout << "Vector Size\tSingle Thread\t2 Threads\tMax Threads\n";
    std::cout << "1M\t" << time1 << "\t" << time2 << "\t" << time3 << "\n";

    // Розраховуємо відносний приріст швидкості для багатопоточного режиму
